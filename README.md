...
Recap:
1) Spring Data JPA: provides hibernate as ORM and HikariCP database connection pool based on entries present in application.properties.
JpaRepository interface, we just need to create our custom interfaces extending this. default CRUD operations are generated by Spring Data JPA.
we can write custom queries using @Query which takes SQL / JP-QL. Also based on methods also it genertates Query like findByFieldName() -- Projections

2) @Transactional: transactional boundary to mark it as unit of work. If no exception occurs in the method marked as @Transactional it commits else rollback.
Any custom INSERT / DELETE or UPDATE used needs to be marked as @Transactional
Any entity which becomes dirty in @Transactional Boundary will be flushed to database by issuing UPDATE SQL.

2) Spring Data Web: provides support for traditional web applications [SSR] and RESTful WS [CSR];
@RestController, @RestMapping, @GetMapping(), @PostMapping(),
@PathVariable, @RequestParam, @RequestBody
@ControllerAdvice with @ExceptionHandler method for centralized exception handling where exceptions are thrown from @Controller / @RestController.
...
* Unit Testing
* Integration Testing
* E2E Testing [ will cover once we complete REACTjs]
...
Above depedency which is added by default for Spring boot application provides:
1) JUnit Testing Framework [ alternate is TestNG ]
2) Mockito library to mock dependencies [ alternates JMock / EasyMock]
3) Hamcrest: assertion library helps assert collections
Collections
...
4) JSONPath: easy to extract JSON data
https://jsonpath.com/

=======================================

The @WebMvcTest annotation is used to create MVC (or more specifically controller) related tests. This creates a Minimalistic Spring Contraner with MVC related beans like HandlerMapping, TestDispatcherServlet. Other layered beans like Service / repository / database connections are not created

@WebMvcTest(VehicleController.class) specifies that VehicleController bean has to be instantiated. Now we don't any other Controllers

Each Test Spec/ test case is marked as  @Test.
